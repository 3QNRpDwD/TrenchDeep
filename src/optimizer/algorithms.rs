...
context_optimizer!(SGD);
context_optimizer!(Momentum);
context_optimizer!(AdaGrad);
context_optimizer!(RMSProp);
context_optimizer!(Adam);
context_optimizer!(AdamW);
...
mod tests {
    ...
    fn parameter_with_gradient(context: &ExecutionContext) -> MlResult<Parameter> {
        let parameter = context.parameter(vec![1.0, -2.0], &[2])?;
    ...
    fn all_context_optimizers_match_their_first_step() -> MlResult<()> {
        ...
        let context = ExecutionContext::new();
        let parameter = parameter_with_gradient(&context)?;
        let mut optimizer = Momentum::new(&context, 0.1, 0.9)?;
        optimizer.register(&parameter)?;
        optimizer.step()?;
        assert_close(&parameter.tensor().to_vec()?, &[0.8, -1.6]);
        ...
        let context = ExecutionContext::new();
        let parameter = parameter_with_gradient(&context)?;
        let mut optimizer = AdaGrad::new(&context, 0.1, 1e-8)?;
        optimizer.register(&parameter)?;
        optimizer.step()?;
        ...
        let context = ExecutionContext::new();
        let parameter = parameter_with_gradient(&context)?;
        let mut optimizer = RMSProp::new(&context, 0.1, 0.9, 1e-8)?;
        optimizer.register(&parameter)?;
        optimizer.step()?;
        ...
        let context = ExecutionContext::new();
        let parameter = parameter_with_gradient(&context)?;
        let mut optimizer = Adam::new(&context, 0.1, 0.9, 0.999, 1e-8)?;
        optimizer.register(&parameter)?;
        optimizer.step()?;
        assert_close(&parameter.tensor().to_vec()?, &[0.9, -1.9]);
        ...
        let context = ExecutionContext::new();
        let parameter = parameter_with_gradient(&context)?;
        let mut optimizer = AdamW::new(&context, 0.1, 0.9, 0.999, 1e-8, 0.1)?;
        optimizer.register(&parameter)?;
        optimizer.step()?;
    ...
    fn registration_rejects_duplicates_and_foreign_contexts() -> MlResult<()> {
        ...
        let parameter = context.parameter(vec![1.0], &[1])?;
