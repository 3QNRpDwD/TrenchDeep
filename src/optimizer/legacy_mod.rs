...
pub trait Optimizer {
    ...
    fn snapshot(&self) -> MlResult<OptimizerSnapshot> {
        Err(crate::legacy::MlError::StringError("optimizer snapshot is not implemented (Phase-6)".into()))
    }
    ...
    fn restore(&mut self, _snapshot: &OptimizerSnapshot) -> MlResult<()> {
        Err(crate::legacy::MlError::StringError("optimizer restore is not implemented (Phase-6)".into()))
    }
