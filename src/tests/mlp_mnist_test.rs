...
use common::training::{build_model, train_model};
use common::utils::{generate_visualization, save_model_parameters, setup_logger};
...
fn mlp_mnist_classification_integration_test() -> MlResult<()> {
    ...
    if accuracy > config.required_accuracy {
        ...
        // 성공 시 모델 파라미터를 저장합니다.
        save_model_parameters(&mlp, &config.model_save_path);
    } else {
        warn!(
