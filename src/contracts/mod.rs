...
impl TensorBuffer {
    pub fn from_vec(data: Vec<f32>, shape: &[usize]) -> MlResult<Self> {
        let expected = shape.iter().try_fold(1usize, |n, &d| n.checked_mul(d))
            .ok_or_else(|| TensorError::InvalidOperation { op: "buffer", reason: "shape size overflow".into() })?;
        if data.len() != expected {
            return Err(TensorError::InvalidDataLength { expected, got: data.len() }.into());
        }
    ...
    pub fn view(&self) -> TensorView<'_> { TensorView { data: &self.data, shape: &self.shape } }
...
impl<'a> TensorView<'a> {
    pub fn new(data: &'a [f32], shape: &'a [usize]) -> MlResult<Self> {
        ...
        if size != Some(data.len()) {
            return Err(TensorError::InvalidOperation { op: "view", reason: "invalid shape or data length".into() }.into());
        }
        Ok(Self { data, shape })
    }
    ...
    pub fn to_owned(self) -> MlResult<TensorBuffer> { TensorBuffer::from_vec(self.data.to_vec(), self.shape) }
}
...
pub fn snapshot(store: &dyn TensorStore, id: TensorId) -> MlResult<TensorBuffer> {
    let mut result = None;
    store.with_view(id, &mut |view| { result = Some(view.to_owned()?); Ok(()) })?;
...
pub trait BackwardOp: Debug {
    ...
    fn backward(&self, inputs: &[TensorView<'_>], saved: &[TensorView<'_>], output_grad: TensorView<'_>)
...
pub enum Reduction { #[default] Mean, Sum, None }
#[derive(Debug, Clone, Copy)]
pub enum LossKind { Mse, Mae, Huber { delta: f32 }, BinaryCrossEntropy, CrossEntropy, SoftmaxCrossEntropy }
impl LossKind {
    pub fn name(self) -> &'static str {
        match self { Self::Mse => "mse_loss", Self::Mae => "mae_loss", Self::Huber {..} => "huber_loss",
            Self::BinaryCrossEntropy => "binary_cross_entropy", Self::CrossEntropy => "cross_entropy",
...
#[derive(Debug, Clone)]
pub enum Operation {
    Add, Sub, Mul, Div, Neg, Square, Exp, Log, Sqrt, Pow(f32), Sin, Cos,
    ApproxSin { threshold: f32 }, ApproxCos { threshold: f32 }, Tanh, Sigmoid, Silu, Relu, Abs,
    Softmax { axis: usize }, Reshape(Vec<usize>), Transpose(Vec<usize>), Concat { axis: usize }, Sum, Matmul,
    ...
    NearestUpsample2d { scale: (usize, usize) }, GroupNorm { groups: usize, epsilon: f32 },
    Loss { kind: LossKind, reduction: Reduction }, TopK { k: usize, sorted: bool },
    Matmax { axis: Option<isize>, keepdim: bool },
}
impl Operation {
    pub fn name(&self) -> &'static str {
        match self {
            Self::Add => "add", Self::Sub => "sub", Self::Mul => "mul", Self::Div => "div", Self::Neg => "neg",
            Self::Square => "square", Self::Exp => "exp", Self::Log => "log", Self::Sqrt => "sqrt", Self::Pow(_) => "pow",
            Self::Sin => "sin", Self::Cos => "cos", Self::ApproxSin {..} => "approx_sin", Self::ApproxCos {..} => "approx_cos",
            Self::Tanh => "tanh", Self::Sigmoid => "sigmoid", Self::Silu => "silu", Self::Relu => "relu", Self::Abs => "abs",
            Self::Softmax {..} => "softmax", Self::Reshape(_) => "reshape", Self::Transpose(_) => "transpose",
            Self::Concat {..} => "concat", Self::Sum => "sum", Self::Matmul => "matmul", Self::Conv2d {..} => "conv2d",
            Self::MaxPool2d {..} => "max_pool2d", Self::AvgPool2d {..} => "avg_pool2d",
            Self::NearestUpsample2d {..} => "nearest_upsample2d", Self::GroupNorm {..} => "group_norm",
            Self::Loss {kind, ..} => kind.name(), Self::TopK {..} => "topk", Self::Matmax {..} => "matmax",
        }
    }
