...
impl ModelState {
    ...
    fn save_json(&self, path: &str) -> MlResult<()> {
        ...
        std::fs::write(path, json)
            .map_err(|e| crate::MlError::StringError(format!("파일 쓰기 실패 '{}': {}", path, e)))
    }

    fn load_json(path: &str) -> MlResult<Self> {
        let json = std::fs::read_to_string(path).map_err(|e| {
            crate::MlError::StringError(format!("파일 읽기 실패 '{}': {}", path, e))
        })?;
        serde_json::from_str(&json)
            .map_err(|e| crate::MlError::StringError(format!("JSON 파싱 실패 '{}': {}", path, e)))
    }
    ...
    fn save_binary(&self, path: &str) -> MlResult<()> {
        ...
        let mut file = std::fs::File::create(path).map_err(|e| {
            crate::MlError::StringError(format!("파일 생성 실패 '{}': {}", path, e))
        })?;
        ...
        file.write_all(MAGIC)
            ...
            .map_err(|e| {
                crate::MlError::StringError(format!("바이너리 쓰기 실패 '{}': {}", path, e))
            })
    }
    ...
    fn load_binary(path: &str) -> MlResult<Self> {
        ...
        if bytes.len() < 16 || &bytes[0..8] != MAGIC {
            return Err(crate::MlError::StringError(format!(
                "'{}' 은 올바른 .tdw 파일이 아님 (magic 불일치)",
                path
            )));
        }

        // 버전 검증
        let file_version = u32::from_le_bytes([bytes[8], bytes[9], bytes[10], bytes[11]]);
        if file_version != FORMAT_VERSION {
            return Err(crate::MlError::StringError(format!(
                "지원하지 않는 .tdw 버전: {} (현재 지원: {})",
        ...
        let mut state: ModelState =
            serde_json::from_slice(&bytes[16..blob_start]).map_err(|e| {
                crate::MlError::StringError(format!("헤더 파싱 실패 '{}': {}", path, e))
            })?;
    ...
    if let Some(parent) = Path::new(path).parent() {
        if !parent.as_os_str().is_empty() {
            std::fs::create_dir_all(parent).map_err(|e| {
                crate::MlError::StringError(format!(
                    "디렉토리 생성 실패 '{}': {}",
                    parent.display(),
                    e
                ))
...
pub(crate) fn find_param<'a>(params: &'a [ParamState], name: &str) -> MlResult<&'a ParamState> {
    params.iter().find(|p| p.name == name).ok_or_else(|| {
        crate::MlError::StringError(format!("파라미터 '{}' 를 체크포인트에서 찾지 못함", name))
    })
}
