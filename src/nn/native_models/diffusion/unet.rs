...
#[cfg(test)]
#[path = "../../../tests/nn/native_models/diffusion/unet_tests.rs"]
mod tests;
    ...
    fn comparison_named_parameters(&self, prefix: &str) -> Vec<(String, &dyn Parameter)> {
        ...
        for branch in [&self.branch1, &self.branch2] {
            for layer in branch.comparison_layers() {
                result.extend(crate::legacy::nn::comparison_layer_parameters(
                    layer.as_ref(),
...
impl SelfAttentionBlock {
    fn comparison_named_parameters(&self, prefix: &str) -> Vec<(String, &dyn Parameter)> {
        let mut result =
            crate::legacy::nn::comparison_layer_parameters(&self.gn, &format!("{prefix}.norm"));
        for (name, layer) in [
            ("q", &self.query),
            ("k", &self.key),
            ("v", &self.value),
...
impl Unet {
    pub fn comparison_named_parameters(&self) -> Vec<(String, &dyn Parameter)> {
        let mut result = crate::legacy::nn::comparison_layer_parameters(self.init_conv.as_ref(), "initial");
