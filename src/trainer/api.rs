...
pub struct EpochSchedule {
    pub epochs: usize,
    pub convergence: super::Convergence,
}
impl EpochSchedule {
    pub fn new(epochs: usize) -> MlResult<Self> {
        if epochs == 0 {
            return Err(MlError::StringError("epochs must be > 0".into()));
        }
        Ok(Self {
            epochs,
            convergence: super::Convergence::Off,
        })
    }
    pub fn with_tolerance(mut self, tolerance: f32) -> Self {
        self.convergence = super::Convergence::from_tolerance(tolerance);
        self
    }
}
    ...
    pub fn new(episodes: usize, max_steps_per_episode: usize) -> MlResult<Self> {
        if episodes == 0 || max_steps_per_episode == 0 {
            return Err(MlError::StringError(
                "episodes and max_steps_per_episode must be > 0".into(),
            ));
        }
        Ok(Self {
            episodes,
...
#[derive(Debug, Clone, Copy)]
pub struct SemiSupervisedOptions {
    pub ramp: super::ConsistencyRamp,
}
#[derive(Debug, Default, Clone, Copy)]
pub struct AutoregressiveOptions {
    pub pad_token_id: Option<usize>,
}
#[derive(Debug, Clone, Copy)]
pub struct ReinforcementOptions {
    pub gamma: f32,
