...
impl BatchInputs for SupervisedBatch {
    fn tensors(&self) -> Vec<&Tensor> {
        vec![self.inputs.tensor(), &self.targets]
    }
}
impl BatchInputs for UnsupervisedBatch {
    fn tensors(&self) -> Vec<&Tensor> {
        vec![self.samples.tensor()]
    }
}
impl BatchInputs for AutoregressiveBatch {
    fn tensors(&self) -> Vec<&Tensor> {
        vec![self.sequences.tensor()]
    }
}
impl BatchInputs for SemiSupervisedBatch {
    fn tensors(&self) -> Vec<&Tensor> {
        vec![
            self.labeled_inputs.tensor(),
            &self.labeled_targets,
            self.unlabeled_inputs.tensor(),
        ]
    }
}

pub(super) fn validate_parameters<M: TrainableModel + ?Sized>(
    ctx: &ExecutionContext,
    model: &M,
    optimizer: &dyn Optimizer,
) -> MlResult<()> {
    if model.context_id() != ctx.id() || optimizer.context_id() != ctx.id() {
        return Err(ContextError::Mismatch.into());
    }
    let expected = model.parameters();
    let actual = optimizer.registered_parameters();
    for p in expected.iter().chain(&actual) {
        ctx.validate(p.tensor())?;
    }
    let expected = expected.iter().map(|p| p.id()).collect::<HashSet<_>>();
    let actual = actual.iter().map(|p| p.id()).collect::<HashSet<_>>();
    if expected != actual {
        return Err(crate::OptimError::ParameterSetMismatch {
...
impl TrainingService {
    ...
    pub fn finish_step<M: TrainableModel>(
        &self,
        model: &M,
        optimizer: &mut dyn Optimizer,
        data: StepData,
        batch: &BatchStartContext,
        forward: Duration,
    ) -> MlResult<StepOutcome> {
        validate_parameters(&self.context, model, optimizer)?;
        self.context.validate(data.loss.tensor())?;
        if let Some(p) = &data.prediction {
            self.context.validate(p.tensor())?;
        }
        if let Some(t) = &data.target {
            self.context.validate(t)?;
        }
        ...
        let snapshot = if let Some(profile) = self.core.requested_capture_profile(batch) {
            Some(self.context.backward_snapshot(
                &data.loss,
        ...
        let backward = backward_start.elapsed();
        ...
        if self.core.config.metrics.grad_norm {
            metrics.insert("grad_norm".into(), squared.sqrt());
        }
        if self.core.config.metrics.update_ratio {
            metrics.insert(
                "update_ratio".into(),
                if weights > 1e-12 {
                    optimizer.lr() * squared.sqrt() / weights.sqrt()
                } else {
                    0.0
                },
            );
        }
        if self.core.config.metrics.fw_bw_timing {
            metrics.insert("forward_secs".into(), forward.as_secs_f32());
            metrics.insert("backward_secs".into(), backward.as_secs_f32());
        }
        ...
        if self.core.hook_count() != 0 {
            let pred = data
                .prediction
                .as_ref()
                .map(|p| p.tensor().snapshot())
                .transpose()?;
            let target = data.target.as_ref().map(Tensor::snapshot).transpose()?;
            let observation = BatchContext {
                batch_idx: batch.batch - 1,
                pred: pred.as_ref(),
                target: target.as_ref(),
                loss,
                n_tokens: data.tokens,
                lambda: data.lambda,
                lr: optimizer.lr(),
            };
            for hook in self.core.hooks.borrow_mut().iter_mut() {
                hook.update(&observation)?;
            }
        }
        optimizer.step()?;
        optimizer.zero_grad()?;
        ...
        F: FnMut(&mut M, I::Batch, usize) -> MlResult<StepData>,
    {
        ...
        let progress =
            progress::EpochProgress::new(schedule.epochs, self.core.config.show_progress);
        let result: MlResult<TrainResult> = (|| {
            ...
            for epoch in 0..schedule.epochs {
                ...
                let batch_progress =
                    progress.start_batch_bar(epoch, schedule.epochs, loader.batch_count());
                let mut sum = 0.0;
                ...
                loop {
                    ...
                    if let Some(snapshot) = outcome.snapshot {
                        self.core.deliver_graph_snapshot(&batch_context, snapshot);
                    }
                    self.core.notify_batch_end(&BatchEndContext {
                        batch: batch_context,
                        loss: outcome.loss,
                    });
                    sum += outcome.loss * outcome.weight as f32;
                    ...
                    if self.core.config.batch_log_interval != usize::MAX
                        && batches % self.core.config.batch_log_interval == 0
                    {
                        batch_progress.set_msg(&format!("loss: {:.6}", outcome.loss));
                    }
                ...
                for hook in self.core.hooks.borrow().iter() {
                ...
                progress.inc();
                if self.core.config.epoch_log_interval != usize::MAX
            ...
            let mut result = TrainResult::epochs(reason, completed, final_loss, started.elapsed())
                .with_metrics(final_metrics);
            if reason == StopReason::Interrupted {
                if let (Some(directory), Some(save)) = (&self.core.config.checkpoint_dir, save) {
            ...
            self.core.notify_train_end(&TrainEndContext {
                ...
                interrupted: reason == StopReason::Interrupted,
            });
            Ok(result)
        })();
        match &result {
            Ok(_) => progress.finish_completed(),
            Err(e) => {
                progress.abandon("training failed");
                self.core.notify_train_error(&e.to_string());
            }
        }
        result
    }
