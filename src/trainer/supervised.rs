...
impl SupervisedTrainer {
    ...
    /// - `input`: `SupervisedBatch` loader 또는 pre-batched `SupervisedDataset`
    /// - `epochs`: 최대 에폭 수
    ...
    where
        M: SupervisedModel,
        L: BatchLoader<Batch = SupervisedBatch>,
    {
...
impl<'a, M: SupervisedModel> EpochStep for SupervisedEpochStep<'a, M> {
    ...
    fn forward_backward(
        ...
        let (y, loss_var) = self.model.forward_loss(&batch.inputs, &batch.targets)?;
        let fw_dur = fw_start.map(|s| s.elapsed());

        let loss = loss_var.tensor().data()[0];

        self.last_y = Some(y);
        self.last_t = Some(batch.targets.clone());
...
#[cfg(test)]
#[path = "../tests/trainer/supervised_tests.rs"]
mod tests;
