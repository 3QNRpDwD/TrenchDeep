...
impl SemiSupervised {
    pub fn new(noise_scale: f32) -> MlResult<Self> {
    ...
    fn noise(&self, input: &Variable) -> MlResult<Tensor> {
        ...
        ctx.tensor(
            ctx.model_uniform(input.tensor().numel()?, self.noise_scale)?,
            &input.tensor().shape()?,
        )
    }
}
impl SemiSupervised {
    ...
    pub fn forward_loss_with_augmentations<L: Loss + ?Sized>(
        &self,
        loss: &L,
        model: &PiClassifier,
        labeled: &Variable,
        ...
        self.forward_with_lambda(
            ...
            first,
            second,
            &ctx.tensor(vec![lambda], &[])?,
        )
    }
    fn forward_with_lambda<L: Loss + ?Sized>(
        &self,
        loss: &L,
        model: &PiClassifier,
        labeled: &Variable,
        ...
        if first.tensor().shape()? != second.tensor().shape()? || first.tensor().numel()? == 0 {
            return Err(invalid(
                "augmented views must have identical, nonempty shapes",
            ));
        }
        let ctx = labeled.tensor().execution_context()?;
        let prediction = model.forward(labeled)?;
        let supervised = loss.compute(&ctx, &prediction, target)?;
...
impl TrainingStrategy<PiClassifier> for SemiSupervised {
    ...
    fn forward_batch<L: Loss + ?Sized>(
        ...
        let lambda = step
            .lambda
            .ok_or_else(|| invalid("semi-supervised step requires lambda"))?;
        let first = batch
            .unlabeled_inputs
            .add(&self.noise(&batch.unlabeled_inputs)?)?;
        let second = batch
            .unlabeled_inputs
            .add(&self.noise(&batch.unlabeled_inputs)?)?;
        let (prediction, loss) = self.forward_loss_with_augmentations(
            loss,
            model,
...
impl PreparedTrainingStrategy<PiClassifier> for SemiSupervised {
    fn execution_batch(
        &self,
        _: &mut PiClassifier,
        batch: &Self::Batch,
    ...
    fn forward_inputs<L: Loss + ?Sized>(
        ...
        let (prediction, loss) = self.forward_with_lambda(
            loss,
            model,
            &inputs.get("labeled")?.as_variable()?,
            inputs.get("target")?,
            &first,
            &second,
            inputs.get("lambda")?,
