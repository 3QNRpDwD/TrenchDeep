...
pub struct StepResult {
    /// 다음 관측치.
    pub next_observation: crate::legacy::tensor::Tensor,
    /// 이 스텝에서 받은 보상.
    pub reward: f32,
    /// 에피소드 종료 여부.
    pub done: bool,
...
pub trait Environment {
    ...
    fn reset(&mut self) -> MlResult<crate::legacy::tensor::Tensor>;
...
pub trait RLModel: TrainableModel {
    ...
    fn policy_logits(
        &mut self,
        obs: &crate::legacy::nn::Variable,
    ) -> MlResult<crate::legacy::nn::Variable>;

    /// No-grad 정책 순전파. 행동 샘플링 시 softmax 적용 전 로짓을 반환한다.
    fn predict_policy_raw(
        &mut self,
        obs: &dyn TensorBase,
    ) -> MlResult<crate::legacy::tensor::GlobalTensor<f32>>;
...
impl RLTrainer {
    ...
    pub fn fit<M: RLModel, E: Environment>(
        ...
        use crate::legacy::tensor::{AutogradFunction, ComputationGraph, Tensor};
        use crate::legacy::nn::Variable;
        use crate::legacy::tensor::operators::{Mul, Function};
        use crate::legacy::loss::SoftmaxCrossEntropyLoss;
...
#[cfg(test)]
#[path = "../tests/trainer/legacy_reinforcement_tests.rs"]
mod tests;
