...
///
/// `params`가 비어 있거나 grad가 아직 계산되지 않았으면 `0.0`을 반환한다.
...
///
/// 현재 학습률에서 한 스텝의 업데이트 크기가 가중치 크기 대비 얼마나 되는지를 나타낸다.
/// 일반적으로 `1e-3` 근처가 건강한 범위이다.
/// `||W|| ≤ 1e-12`이면 `0.0`을 반환한다 (제로 나눗셈 방지).
...
///
/// 조기 종료(short-circuit)로 구현되어 첫 비정상값 발견 시 즉시 반환한다.
...
///
/// # 주의
/// argmax가 유효하지 않은 샘플(all-zero 등)은 분모에서 제외한다.
/// 이는 손실의 분모(`total_loss_count`)와 다를 수 있으므로 두 값을 혼용하지 않도록 한다.
#[derive(Default)]
...
impl ClassificationAccuracy {
    ...
    /// 현재까지 누적된 정확도를 백분율(0.0 ~ 100.0)로 반환한다.
...
/// 데이터 슬라이스에서 최대값의 인덱스를 반환한다.
///
/// 동률 시 첫 번째 최대값의 인덱스를 반환한다.
/// 슬라이스가 비어 있으면 `None`을 반환한다.
