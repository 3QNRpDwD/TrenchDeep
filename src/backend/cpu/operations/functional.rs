...
pub(crate) fn topk_forward_data(
    input: TensorView<'_>,
    k: usize,
    sorted: bool,
...
pub(crate) fn matmax_forward_data(
    input: TensorView<'_>,
    axis: Option<isize>,
    keepdim: bool,
) -> MlResult<(Vec<f32>, Vec<f32>, Vec<usize>)> {
...
pub(crate) fn validate_approx_threshold(operation: &'static str, threshold: f32) -> MlResult<()> {
...
pub(crate) fn validate_loss_pair(
    kind: LossKind,
    ...
    match kind {
        ...
        LossKind::CrossEntropy | LossKind::SoftmaxCrossEntropy => {
            ...
            if matches!(kind, LossKind::CrossEntropy)
                && prediction
                    .data
                    .iter()
...
pub(crate) fn reduce_loss_values(
    values: Vec<f32>,
    none_shape: &[usize],
    reduction: Reduction,
) -> MlResult<TensorBuffer> {
    match reduction {
        Reduction::None => TensorBuffer::from_vec(values, none_shape),
        Reduction::Sum => TensorBuffer::from_vec(vec![values.iter().sum()], &[]),
        Reduction::Mean => {
            ...
            TensorBuffer::from_vec(vec![values.iter().sum::<f32>() / count as f32], &[])
        }
    }
}

pub(crate) fn loss_forward(
    kind: LossKind,
    reduction: Reduction,
    prediction: TensorView<'_>,
    target: TensorView<'_>,
    save_softmax: bool,
) -> MlResult<(TensorBuffer, Option<TensorBuffer>)> {
    validate_loss_pair(kind, prediction, target)?;
    let mut saved = None;
    let output = match kind {
        LossKind::Mse => reduce_loss_values(
            prediction
                .data
                .iter()
                .zip(target.data)
        ...
        LossKind::Mae => reduce_loss_values(
            prediction
                .data
                .iter()
                .zip(target.data)
        ...
        LossKind::Huber { delta } => reduce_loss_values(
            prediction
                .data
                .iter()
                .zip(target.data)
        ...
        LossKind::BinaryCrossEntropy => reduce_loss_values(
            prediction
                .data
                .iter()
                .zip(target.data)
        ...
        LossKind::CrossEntropy | LossKind::SoftmaxCrossEntropy => {
            ...
            {
                let loss = if matches!(kind, LossKind::CrossEntropy) {
                    -prediction_row
                        .iter()
                        .zip(target_row)
            ...
            if let Some(probabilities) = probabilities {
                saved = Some(TensorBuffer::from_vec(probabilities, prediction.shape)?);
            }
...
pub(crate) fn loss_backward(
    ...
    TensorBuffer::from_vec(gradient, prediction.shape)
}

pub(crate) fn approx_sin_value(x: f32) -> f32 {
    let mut result = x;
    let mut sign = -1.0;
    let mut power = 3_u32;
...
pub(crate) fn approx_cos_value(x: f32) -> f32 {
    let x_squared = x * x;
    let mut result = 1.0;
    let mut sign = -1.0;
    let mut power = 2_u32;
...
pub(crate) fn approx_sin_derivative(x: f32) -> f32 {
    approx_cos_value(x)
}

pub(crate) fn approx_cos_derivative(x: f32) -> f32 {
    let x_squared = x * x;
    let mut result = -x;
    let mut sign = 1.0;
    let mut power = 3_u32;
