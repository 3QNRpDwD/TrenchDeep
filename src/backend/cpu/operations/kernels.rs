...
pub(crate) fn group_norm_forward_data(
    input: &TensorBuffer,
    gamma: &TensorBuffer,
    beta: &TensorBuffer,
    groups: usize,
    epsilon: f32,
) -> MlResult<(TensorBuffer, Vec<TensorBuffer>)> {
    ...
    Ok((
        TensorBuffer::from_vec(output, &input.shape)?,
        vec![
            TensorBuffer::from_vec(normalized, &input.shape)?,
            TensorBuffer::from_vec(means, &[n, groups])?,
            TensorBuffer::from_vec(variances, &[n, groups])?,
        ],
    ))
}

pub(crate) fn group_norm_backward_data(
    input: &TensorBuffer,
    gamma: &TensorBuffer,
    saved: &[TensorView<'_>],
    grad: &TensorBuffer,
    ...
    Ok((
        ...
        TensorBuffer::from_vec(dgamma, &gamma.shape)?,
        TensorBuffer::from_vec(dbeta, &beta_shape)?,
    ))
}
...
pub(crate) fn nearest_upsample2d_forward_data(
    input: &TensorBuffer,
    scale: (usize, usize),
    ...
    TensorBuffer::from_vec(output, &[n, c, oh, ow])
}

pub(crate) fn nearest_upsample2d_backward_data(
    input: &TensorBuffer,
    grad: &TensorBuffer,
    scale: (usize, usize),
) -> MlResult<TensorBuffer> {
...
pub(crate) fn max_pool2d_forward_data(
    input: &TensorBuffer,
    kernel: (usize, usize),
    stride: (usize, usize),
) -> MlResult<(TensorBuffer, TensorBuffer)> {
    ...
    let shape = [n, c, oh, ow];
    Ok((
        TensorBuffer::from_vec(output, &shape)?,
        TensorBuffer::from_vec(mask, &shape)?,
    ))
}

pub(crate) fn max_pool2d_backward_data(
    input: &TensorBuffer,
    mask: &TensorView<'_>,
    grad: &TensorBuffer,
    kernel: (usize, usize),
...
pub(crate) fn avg_pool2d_forward_data(
    input: &TensorBuffer,
    kernel: (usize, usize),
    stride: (usize, usize),
) -> MlResult<TensorBuffer> {
...
pub(crate) fn avg_pool2d_backward_data(
    input: &TensorBuffer,
    grad: &TensorBuffer,
    kernel: (usize, usize),
    stride: (usize, usize),
...
pub(crate) fn conv2d_forward_data(
    input: &TensorBuffer,
    weight: &TensorBuffer,
    bias: &TensorBuffer,
    stride: (usize, usize),
...
impl MatmulSpec {
    pub(crate) fn new(left: &[usize], right: &[usize]) -> MlResult<Self> {
...
pub(crate) fn validate_permutation(shape: &[usize], axes: &[usize]) -> MlResult<()> {
...
pub(crate) fn permute_data(data: &[f32], input_shape: &[usize], axes: &[usize]) -> Vec<f32> {
...
pub(crate) fn broadcast_shape(lhs: &[usize], rhs: &[usize]) -> Option<Vec<usize>> {
...
pub(crate) fn broadcast_data(input: &TensorBuffer, output_shape: &[usize]) -> MlResult<Vec<f32>> {
...
pub(crate) fn reduce_to_shape(
    ...
    TensorBuffer::from_vec(data, target_shape)
}

pub(crate) fn tensor_map(tensor: &TensorBuffer, f: impl Fn(f32) -> f32) -> MlResult<TensorBuffer> {
    TensorBuffer::from_vec(tensor.data.iter().copied().map(f).collect(), &tensor.shape)
}

pub(crate) fn tensor_zip(
    lhs: &TensorBuffer,
    rhs: &TensorBuffer,
