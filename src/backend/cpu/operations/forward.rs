    ...
    fn execute(
        ...
        let (output, backward) = match operation {
            Operation::Add | Operation::Sub | Operation::Mul | Operation::Div => {
                let y = &values[1];
                let shape = broadcast_shape(&x.shape, &y.shape).ok_or_else(|| {
                    TensorError::InvalidOperation {
                        op: operation.name(),
                        reason: "incompatible broadcast shapes".into(),
                    }
                })?;
                let a = broadcast_data(x, &shape)?;
                let b = broadcast_data(y, &shape)?;
                let data = match operation {
                    Operation::Add => self.compute.add(&a, &b)?,
                    Operation::Sub => self.compute.sub(&a, &b),
                    Operation::Mul => self.compute.multiply(&a, &b)?,
                    _ => self.compute.div(&a, &b),
                };
                let backward = match operation {
                    Operation::Add => BuiltinBackward::Add,
                    Operation::Sub => BuiltinBackward::Sub,
                    Operation::Mul => BuiltinBackward::Mul,
                    _ => BuiltinBackward::Div,
                };
            ...
            Operation::Exp => (
                TensorBuffer::from_vec(self.compute.exp(&x.data), &x.shape)?,
                BuiltinBackward::Exp,
            ),
            Operation::Log => (
                TensorBuffer::from_vec(self.compute.log(&x.data), &x.shape)?,
                BuiltinBackward::Log,
            ),
            Operation::Sqrt => (
                TensorBuffer::from_vec(self.compute.sqrt(&x.data), &x.shape)?,
                BuiltinBackward::Sqrt,
            ),
            Operation::Pow(p) => (
                TensorBuffer::from_vec(self.compute.pow(&x.data, *p), &x.shape)?,
                BuiltinBackward::Pow(*p),
            ),
            ...
            Operation::Sigmoid => (
                tensor_map(x, |x| 1.0 / (1.0 + (-x).exp()))?,
                BuiltinBackward::Sigmoid,
            ),
            Operation::Silu => (
                tensor_map(x, |x| x / (1.0 + (-x).exp()))?,
            ...
            Operation::Softmax { axis } => {
                ...
                for o in 0..outer {
                    for i in 0..inner {
                        let max = (0..width)
                            .map(|j| x.data[(o * width + j) * inner + i])
                            .fold(f32::NEG_INFINITY, f32::max);
                        let sum: f32 = (0..width)
                            .map(|j| (x.data[(o * width + j) * inner + i] - max).exp())
                            .sum();
                        for j in 0..width {
                            let idx = (o * width + j) * inner + i;
                            data[idx] = (x.data[idx] - max).exp() / sum;
                        }
                    }
                }
                (
                    TensorBuffer::from_vec(data, &x.shape)?,
                    BuiltinBackward::Softmax { axis },
                )
            }
            Operation::Sum => (
                TensorBuffer::from_vec(vec![self.compute.sum(&x.data)], &[])?,
                BuiltinBackward::Sum,
            ),
            Operation::Reshape(shape) => (
                TensorBuffer::from_vec(x.data.clone(), shape)?,
            ...
            Operation::Transpose(axes) => {
                ...
                (
                    TensorBuffer::from_vec(permute_data(&x.data, &x.shape, axes), &shape)?,
                    BuiltinBackward::Transpose(axes.clone()),
                )
            }
            Operation::Concat { axis } => {
                ...
                for value in &values {
                    if value.shape.len() != x.shape.len()
                        || value
                            .shape
                            .iter()
                            .enumerate()
                            .any(|(i, &d)| i != axis && d != x.shape[i])
                    {
                        return Err(TensorError::InvalidOperation {
                            op: "concat",
                ...
                shape[axis] = values
                    .iter()
                    .try_fold(0usize, |n, v| n.checked_add(v.shape[axis]))
                ...
                (
                    TensorBuffer::from_vec(data, &shape)?,
                    BuiltinBackward::Concat {
                        axis,
                        sizes: values.iter().map(|v| v.shape[axis]).collect(),
                    },
                )
            }
            Operation::Matmul => {
                ...
                (
                    TensorBuffer::from_vec(data, &spec.output_shape)?,
                    BuiltinBackward::Matmul,
                )
            }
            Operation::Conv2d { stride, padding } => (
                conv2d_forward_data(x, &values[1], &values[2], *stride, *padding)?,
            ...
            Operation::AvgPool2d { kernel, stride } => (
                avg_pool2d_forward_data(x, *kernel, *stride)?,
            ...
            Operation::NearestUpsample2d { scale } => (
                nearest_upsample2d_forward_data(x, *scale)?,
                BuiltinBackward::NearestUpsample2d { scale: *scale },
            ),
            Operation::GroupNorm { groups, epsilon } => {
                let (out, s) =
                    group_norm_forward_data(x, &values[1], &values[2], *groups, *epsilon)?;
                saved = s;
                (
                    out,
            ...
            Operation::TopK { k, sorted } => {
                ...
                return Ok(OperationOutput {
                    outputs: vec![
                        TensorBuffer::from_vec(v, &s)?,
                        TensorBuffer::from_vec(i, &s)?,
                    ],
                    saved,
                    backward: None,
                });
        ...
        Ok(OperationOutput {
            outputs: vec![output],
            saved,
            backward: Some(into_node_backward(backward)),
        })
    }
}
