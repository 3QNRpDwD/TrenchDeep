//! Native graph implementation, built in the same crate as P1.
#[path = "backend/legacy_mod.rs"]
pub mod backend;
#[path = "loss/mod.rs"]
pub mod loss;
#[path = "nn/legacy_mod.rs"]
pub mod nn;
#[path = "optimizer/legacy_mod.rs"]
pub mod optimizer;
#[path = "tensor/mod.rs"]
pub mod tensor;
#[path = "trainer/legacy_mod.rs"]
pub mod trainer;
pub use loss::Reduction;
#[cfg(feature = "enableVisualization")]
#[path = "visualization/legacy_mod.rs"]
pub mod visualization;
#[path = "tests/mod.rs"]
pub mod tests;

use crate::legacy::backend::BackendError;
use crate::legacy::loss::LossError;
use crate::legacy::optimizer::OptimError;
use thiserror::Error;
...
pub enum ContextError {
    ...
    UnknownTensor(crate::legacy::tensor::NodeId),
...
pub enum AutogradError {
    ...
    GraphAlreadyFreed(crate::legacy::tensor::NodeId),
    #[error("Computation node {0:?} was not found")]
    NodeNotFound(crate::legacy::tensor::NodeId),
...
pub enum MlError {
    ...
    #[error(transparent)]
    DataError(#[from] crate::legacy::trainer::data::DataError),
    #[cfg(feature = "enableVisualization")]
    #[error(transparent)]
    VisualizationError(#[from] crate::legacy::visualization::VisualizationError),
}
...
mod benchmark {
    ...
    use crate::legacy::tensor::operators::{Add, Function, Mul, Square};
    use crate::legacy::tensor::{AutogradFunction, ComputationGraph, Tensor, TensorBase};
    use crate::legacy::tests::common::logging::setup_logging;
    use crate::legacy::{MlResult, scalar, var_input, var_with_label, variable};
    ...
    fn goldstein() -> MlResult<()> {
        #[cfg(feature = "enableVisualization")]
        let capture = crate::legacy::visualization::VisualizationCapture::builder(
            crate::legacy::visualization::CaptureProfile::Analysis,
        )
        .begin()?;
        ...
        #[cfg(feature = "enableVisualization")]
        {
            ...
            use crate::legacy::visualization::SnapshotWriter;
            let directory =
                std::env::temp_dir().join(format!("trench-deep-goldstein-{}", std::process::id()));
            let mut writer =
                crate::legacy::visualization::FileSnapshotWriter::builder(&directory).build()?;
...
#[path = "comparison.rs"]
pub mod comparison;
pub use crate::{variable, tensor_ops, scalar_ops, scalar, register_operator, var_input, var_output, var_act, var_weight, var_bias, var_with_label};
