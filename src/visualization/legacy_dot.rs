...
mod tests {
    use super::*;
    use crate::legacy::visualization::{
    ...
    fn overview_hides_saved_nodes_but_detailed_keeps_them() {
        let mut input = node(1, NodeRole::Input);
        input.shape = vec![2];
        input.elements = 2;
        input.estimated_bytes = 8;
        let snapshot = GraphSnapshot {
            schema_version: GRAPH_SNAPSHOT_SCHEMA_VERSION,
            profile: CaptureProfile::Structure,
            context: CaptureContext::default(),
            nodes: vec![input, node(2, NodeRole::Saved), node(3, NodeRole::Input)],
            edges: Vec::new(),
            attributes: BTreeMap::new(),
        };
        ...
        assert!(!overview.contains("id=\"node-3\""));
        assert!(overview.contains("2 auxiliary nodes hidden"));
        let detailed = DotEncoder::encode_with_profile(&snapshot, DotProfile::Detailed);
        assert!(detailed.contains("id=\"node-2\""));
        assert!(detailed.contains("id=\"node-3\""));
    }
