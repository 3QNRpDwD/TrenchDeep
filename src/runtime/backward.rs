...
impl ExecutionContext {
    ...
    pub(super) fn backward_observed(
        ...
        if !state.tracked.contains(&root) {
            return Err(AutogradError::NodeNotFound(root).into());
        }
        if state.consumed.contains(&root) {
            return Err(AutogradError::GraphAlreadyFreed(root).into());
        }
        let value = state.snapshot(root)?;
        let seed = if let Some(gradient) = options.gradient {
            if gradient.context_id() != self.id {
            ...
            if value.data.len() != 1 {
                return Err(AutogradError::OutputNotScalar(value.shape).into());
            }
        ...
        for &id in &order {
            if !seen.insert(id) && records.iter().any(|n: &GradientRecord| n.output == id) {
            ...
            seen.extend(record.inputs.iter().copied());
            records.push(record);
        }
        state.gradients.retain(|id, _| !seen.contains(id));
        state.gradients.insert(root, seed);
        for record in records.iter().rev() {
            let Some(grad) = state.gradients.get(&record.output).cloned() else {
                continue;
            };
            let inputs = record
                .inputs
            ...
            let incoming = record
                .backward
                .backward(&views, &saved_views, grad.view())?;
            if incoming.len() != record.inputs.len() {
                return Err(AutogradError::BackwardArityMismatch {
                    expected: record.inputs.len(),
                    got: incoming.len(),
                }
                .into());
            }
            // Validate the complete VJP before applying any of its gradients.
            for (input, g) in inputs.iter().zip(&incoming) {
                if let Some(g) = g {
            ...
            for (&id, g) in record.inputs.iter().zip(incoming) {
                if let Some(g) = g {
                    if !state.tracked.contains(&id) {
                        continue;
                    }
                    if let Some(old) = state.gradients.get_mut(&id) {
                        for (a, b) in old.data.iter_mut().zip(g.data) {
                            *a += b;
                        }
                    } else {
                        state.gradients.insert(id, g);
                    }
                }
            }
        }
        ...
        if !options.retain_graph {
            for record in &records {
                state.remove_graph(record.output)?;
                state.consumed.insert(record.output);
            }
            state.consumed.insert(root);
            state.collect()?;
        }
        let keep = state
            .leaves
